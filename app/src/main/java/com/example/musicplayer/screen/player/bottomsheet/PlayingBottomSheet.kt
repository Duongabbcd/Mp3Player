...
import com.example.musicplayer.databinding.BottomSheetPlayingBinding
import com.example.musicplayer.screen.home.dialog.DeleteSongDialog
import com.example.musicplayer.screen.player.PlayingArtistActivity
...
class PlayingBottomSheet (private val context: Context) :
    BaseBottomSheetDialog<BottomSheetPlayingBinding>(context) {
    private val listArtist by lazy {
        SongLoader.getArtistsInDevice(context )
    }
       private  var song: Audio = Audio.EMPTY_SONG

    fun setPlayingAudio(currentAudio: Audio) {
        song = currentAudio
        binding.apply {
            ...
            delete.setOnClickListener {
                val deleteDialog = DeleteSongDialog(context)
                deleteDialog.deleteAudioFromDevice(song)
                deleteDialog.show()
                dismiss()
            }
            ...
            checkArtist.setOnClickListener {
                val artistOfSong = listArtist.filter { it.nameArtist == currentAudio.artistName }
                if (artistOfSong.isNullOrEmpty()) {
                    MDManager.getInstance(context)
                        ?.showMessage(context, "Can not find the artist of this song!")
                } else {
                    val intent = Intent(context, PlayingArtistActivity::class.java)
                    PlayingArtistActivity.artist = artistOfSong.first()
                    context.startActivity(intent)
                }
            }

            speed.setOnClickListener {
                val dialog = SpeedBottomSheet(context) { result ->
                    MusicPlayerRemote.setSpeed(context, result)
                    MusicPlayerRemote.getSettingSpeed(result)
//                    context.sendBroadcast(Intent(PlayerActivity.PLAY_SPEED))
                }
                dialog.show()
                dismiss()
            }
            ...
            playlist.setOnClickListener {
                if(song != Audio.EMPTY_SONG) {
                    val addPlaylistBottomSheet = AddPlaylistBottomSheet(context)
                    addPlaylistBottomSheet.setData(song)
    ...
    override fun getViewBinding(): BottomSheetPlayingBinding {
        println("currentAudio: $song")
        return BottomSheetPlayingBinding.inflate(layoutInflater)
    }
