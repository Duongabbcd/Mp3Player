...
object MusicPlayerRemote {
    ...
    fun openQueue(queue: ArrayList<Any>, startPosition: Int, startPlaying: Boolean, isNotRemember: Boolean? = null ) {

        val result = tryToHandleOpenPlayingQueue(queue, startPosition, startPlaying)
        if (!result) {
            musicService?.openQueue(queue, startPosition, startPlaying, isNotRemember)
            if (musicService == null) return
            Log.d("openQueue","openQueue: ${PreferenceUtil.getInstance(musicService!!)?.rememberShuffle()}")
            if (PreferenceUtil.getInstance(musicService!!)?.rememberShuffle() == true && isNotRemember == false) {
    ...
    fun setRecentlyPlayedSongs(audio: Audio) {
        val allSongs =
            PreferenceUtil.getInstance(musicService!!)?.getAllRecentlyPlayed() ?: listOf()

        val saved = allSongs.toMutableList()
        if(saved.none { it.mediaObject?.path == audio.mediaObject?.path }) {
            saved.add(audio)
        ...
        PreferenceUtil.getInstance(musicService!!)?.setAllRecentlyPlayed(Gson().toJson(saved))
    }
    ...
    fun openAndShuffleQueue(queue: ArrayList<Any>, startPlaying: Boolean) {
        ...
        if (!tryToHandleOpenPlayingQueue(
            ...
            openQueue(queue, startPosition, startPlaying, isNotRemember = true)
        }
    }
    ...
    fun toggleShuffleMode(shuffleMode: Int? = null) {
        musicService?.toggleShuffle(shuffleMode)
    }

    fun getSettingSpeed(speed : Float = 1f) {
    ...
    fun setSpeed(context: Context, speed: Float = 1.0f) {
        val preferences = context.getSharedPreferences(
            context.packageName,
            Context.MODE_MULTI_PROCESS
        )
