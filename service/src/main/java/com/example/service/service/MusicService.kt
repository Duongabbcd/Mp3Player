...
class MusicService : Service(), SharedPreferences.OnSharedPreferenceChangeListener,
    ...
    private fun restoreState() {
        shuffleMode =
            getDefaultSharedPreferences(this).getInt(SAVED_SHUFFLE_MODE, 0)
        repeatMode =
            getDefaultSharedPreferences(this).getInt(SAVED_REPEAT_MODE, 0)
    ...
    private fun setDataSourceImpl(currentSong: Audio): Boolean {
        try {
            ...
            playback = ExoPlayer.Builder(this).build()
            val currentSpeed = MusicPlayerRemote.getSpeed(this)
            MusicPlayerRemote.getSettingSpeed(currentSpeed)
    ...
    fun openQueue(queue: ArrayList<Any>?, startPosition: Int, startPlaying: Boolean, isNotRemember: Boolean? = false) {
        if (!queue.isNullOrEmpty() && startPosition >= 0 && startPosition < queue.size) {
            // it is important to copy the playing queue here first as we might add/remove songs later
            println("openQueue: $queue")
            println("openQueue 1: $startPosition")
            println("openQueue 2: $startPlaying")
            originalPlayingQueue = ArrayList(queue)
            playingQueue = originalPlayingQueue?.let { ArrayList(it) }!!

            var position = startPosition
            if (isNotRemember == true || shuffleMode == SHUFFLE_MODE_SHUFFLE) {
    ...
    private fun playSongAtImpl(position: Int) {
        try {
            ...
            if (openTrackAndPrepareNextAt(position)) {
                play()
                MusicPlayerRemote.setRecentlyPlayedSongs(getCurrentSong())
            } else {
    ...
    private class PlaybackHandler(val service: MusicService) : Handler() {
        ...
        private fun smoothSetVolume(service: MusicService, toVolume: Float) {
            val player = service.playback ?: return
            ...
            Thread {
                ...
                while (abs(volume - toVolume) > 0.01f) {
                    volume += if (volume < toVolume) step else -step
            ...
            when (msg.what) {
                DUCK -> {
                    if (!isDucking) {
                        isDucking = true
                        targetVolume = 0.2f
                        smoothSetVolume(service, targetVolume)
                    }
                }
    ...
    fun toggleShuffle(shuffleMode: Int ? = null) {
        if(shuffleMode != null) {
    ...
    fun applyLoudnessEnhancerIfNeeded() {
        val sessionId = playback?.audioSessionId ?: return
        try {
            val enhancer = LoudnessEnhancer(sessionId)
            enhancer.setTargetGain(1000) // 10 dB là đẹp
            enhancer.enabled = true
            applyEqualizerIfNeeded()
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }

    fun applyEqualizerIfNeeded() {
        try {
            val sessionId = playback?.audioSessionId ?: return
            val equalizer = Equalizer(0, sessionId)
            equalizer.enabled = true

            // Ví dụ: Tăng bass (low band), giảm mid
            val bands = equalizer.numberOfBands.toInt()
            val bassBand = 0 // thường là band đầu
            val trebleBand = bands - 1 // band cuối

            equalizer.setBandLevel(bassBand.toShort(), 1000) // tăng 1dB
            equalizer.setBandLevel(trebleBand.toShort(), 500) // tăng nhẹ treble
    ...
    fun setSpeed(speed: Float) {
        playback?.playbackParameters = PlaybackParameters(speed)
    }
    ...
    companion object {
        ...
        const val MUSIC_PACKAGE_NAME = "com.offlinemusicplayer.playmusic.musicapp"
