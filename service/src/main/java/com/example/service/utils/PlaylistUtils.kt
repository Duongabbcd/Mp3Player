...
class PlaylistUtils(private val context: Context) {
    ...
    fun checkIsFavourite(audio: Audio): Boolean {
        ...
        if (favPlaylist.tracks.isNotEmpty()) {
            for (i in 0 until favPlaylist.tracks.size) {
                ...
                if (favPlaylist.tracks[i].mediaObject?.path == audio.mediaObject?.path) {
                    println("checkIsFavourite 0: $audio")
                    println("checkIsFavourite 1: ${favPlaylist.tracks[i]}")
                    return true
                } else {
                    println("checkIsFavourite 0: $audio")
                    println("checkIsFavourite 2: ${favPlaylist.tracks[i]}")
                }
            }
        }
        return false
    ...
    suspend fun updatePlaylistInfo(playlist: Playlist, imageThumb: Uri, editedName: String) {
        try {
